...
import tests.hackerrank.stub.Player;
 ...
 * players have the same score, sort those players alphabetically ascending by name. To do this, you must create a
 * Checker class that implements the Comparator interface, then write an int compare(Player a, Player b) method
 * implementing the Comparator.compare(T o1, T o2) method. In short, when sorting in ascending order, a comparator
...
public class Checker implements Comparator<Player> {
    ...
    public int compare(Player a, Player b) {
        ...
        if (diff == 0) return a.name.compareTo(b.name);
        return diff;
    }
}
